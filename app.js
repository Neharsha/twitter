...
(username,password,gender,name) values("${username}","${hasshedPassword}","${gender}","${name}");`);
      response.status(200);
...
app.post("/login/", async (request, response) => {
  ...
  if (dbUser !== undefined) {
    ...
    if (isPasswordMatch) {
      let jwtToken = jwt.sign({ username: username }, "MY_SECRET_KEY");
...
//all the people who logged in user if following
...
select
user.name ,reply.reply from reply natural join user where tweet_id=${tweetId};`);
...
select tweet.tweet,
count(distinct like.like_id) as likes,count(distinct reply.reply_id) as replies,tweet.date_time from
tweet
